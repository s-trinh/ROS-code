...
public:
  Visualization() : m_nh(), m_queue_size(10), m_image_color_sub(m_nh, "/camera/color", m_queue_size),
      m_image_depth_sub(m_nh, "/camera/depth", m_queue_size), m_pointcloud_sub(m_nh, "/camera/pointcloud", m_queue_size),
      // ApproximateTime takes a queue size as its constructor argument, hence MySyncPolicy(10)
      m_sync(MySyncPolicy(10), m_image_color_sub, m_image_depth_sub, m_pointcloud_sub),
      m_I_depth(), m_viewer_thread(), m_mutex(), m_viewer(false, m_mutex)
  {
    m_sync.registerCallback(boost::bind(&Visualization::callback, this, _1, _2, _3));
  ...
  void callback(const sensor_msgs::ImageConstPtr& image_color, const sensor_msgs::ImageConstPtr& image_depth,
                const sensor_msgs::PointCloud2ConstPtr& msg_pointcloud) {
    try {
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "RGBD_Visualization_opencv_node");
